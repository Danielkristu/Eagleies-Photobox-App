...
def use_voucher(doc_id):
    ...
    return redirect(f"/voucher_payment_qris/{doc_id}/{discount}")
...
def start_payment_qris_voucher(doc_id):
    ...
    if not xendit_client:
        ...
        return {"error": "API client could not be configured"}, 500
    try:
        ...
        return {"error": "Gagal membuat kode QRIS", "details": response.text}, 500
    except Exception as e:
        print(f"[VOUCHER_QRIS] Error creating payment: {str(e)}")
        return {"error": "Terjadi kesalahan sistem"}, 500


@voucher_bp.route("/voucher_payment_qris/<doc_id>/<int:discount>")
def voucher_payment_qris(doc_id, discount):
    # Render QRIS page for voucher, passing discount directly
    config = db_fs.collection("Clients").document(session.get("client_id")).collection("Booths").document(doc_id).get().to_dict()
    print(f"[VOUCHER_QRIS_PAGE] discount: {discount}")
    ...
    import time
    cache_buster = int(time.time())
    return render_template("payment_qris.html", doc_id=doc_id, price_per_session=price_per_session, bg_url=local_bg_path, cache_buster=cache_buster, voucher_discount=discount)
